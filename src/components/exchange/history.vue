    ...
    <div class="table-wrap">
        <table>
            ...
            <tr v-for="record in recordList">
                ...
                <td class="integral" :class="parseInt(record.score) > 0 ? 'add' : 'minus'">{{parseInt(record.score) > 0 ? "+" + record.score : record.score}}</td>
                <td class="telphone">{{record.telphone}}</td>
            </tr>
            </template>
            <tr v-else>
                <td colspan="4" class="null-list">
                    <h1 class="hd"><i class="icon-null"></i>对不起，您暂无任何积分兑换记录！</h1>
                    <p class="tips">（ 现在就去 <a v-link="{path:'/u/exchange/cashing/'}">兑换</a>商品~ ）</p>
                </td>
            </tr>
            </tbody>
        </table>
...
<script>
    ...
    methods:{
        init(cur){
            ...
            var that = this;
            var url = config.API_BASE+"/4s/score/list";
            var query = {};
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    that.$set("recordList",response.data.rows);
